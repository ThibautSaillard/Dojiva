...
Rule: after a design subagent restyles existing pages, always (1) run the package typecheck and (2) diff the `data-testid` inventory (`grep -o 'data-testid="[^"]*"' | sort -u`) against the pre-pass state — even when the brief explicitly says to keep them.
